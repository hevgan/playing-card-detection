...
def normalizeCardCollection(collection):
    ...
    global cards
    figures = [cards[sorted_by_occurences[i][0]] for i in range(len(sorted_by_occurences))]
    return figures
...
class CardRecognizer():
    def __init__(self, network ):
        ...
        cv.resizeWindow(self.window_name, 600, 400)
    ...
    def processOutput(self, frame, outs):
        ...
        found_cards = list((sorted(set([self.classes[class_ids[i]] for i in range(len(class_ids))]))))
